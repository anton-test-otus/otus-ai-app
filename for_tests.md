...
## Compliance C3 — дашборд со статистикой (B+)

**Источник:** `COMPLIANCE_PLAN.md`, шаг C3 (вариант B+: KPI + 2 графика)

### Smoke (ручная проверка)
- [x] Multi-user: логин `hogwarts@demo.local` / `demo1234`
- [x] В сайдбаре при наведении на строку «Настройки» / email — кнопка «Статистика» (chart-bar); клик → `/stats`
- [x] На `/stats`: 6 KPI-карточек и 2 графика (doughnut по папкам, bar по тегам)
- [x] Цифры правдоподобны для demo seed
- [x] На `/` (список заметок) блок статистики **не** отображается
- [x] `GET /api/stats` с JWT возвращает JSON с `notesCount`, `notesByFolder`, `topTags`
- [x] Клик по сегменту doughnut → `/` с выбранной папкой в сайдбаре; клик по bar-тегу → `/` с фильтром по тегу

**Ожидание:** статистика только на отдельной странице; доступ из hover-кнопки у настроек аккаунта.

### Автотесты (позже)
- Functional: `GET /api/stats` для пользователя с заметками/тегами/ссылками

---

## Compliance C5 — 15 тегов в demo seed

**Источник:** `COMPLIANCE_PLAN.md`, шаг C5

### Smoke (ручная проверка)
- [x] `docker compose exec php php bin/console app:seed-demo-data --force` — в выводе для каждой вселенной `Тегов: 15`
- [x] Логин `hogwarts@demo.local` / `demo1234` → облако тегов / сайдбар: ≥15 уникальных тегов (в т.ч. `школа`, `магия`, `смерть`, `дружба`, `пророчество`)
- [x] Фильтр «И»: выбрать `персонаж` + `дружба` — в списке есть заметки (напр. «Гарри Поттер», «Рон Уизли»)
- [x] `/stats` — bar «популярные теги» отражает новые теги

**Ожидание:** 15 тегов на каждого demo-пользователя; новые теги назначены заметкам и видны в UI.

---

## Compliance C4 — полнотекстовый поиск (PostgreSQL FTS)

**Источник:** `COMPLIANCE_PLAN.md`, шаг C4

### Smoke (ручная проверка)
- [ ] Логин `hogwarts@demo.local` / `demo1234`
- [ ] SearchBar: `хогвартс` — несколько заметок; `факульт` → заметки со словом «факультет» (префикс FTS)
- [ ] SearchBar: `патронус` → «Гарри Поттер» и др.
- [ ] Регистронезависимость: `хогвартс` и `ХОГВАРТС` — одинаковый набор
- [ ] Логин `westeros@demo.local` → SearchBar: `свадьба` → «Red Wedding» и др.
- [ ] Поиск + фильтр папки / тега в сайдбаре — список сужается
- [ ] Wiki-модалка: поиск по title (`GET /api/notes?title=…`) без регрессии

**Не проверяем:** имена тегов (`пророчество`, `предательство` и т.д.) в SearchBar — фильтрация по тегам через облако/сайдбар, FTS только по тексту заметки.

**Ожидание:** FTS (`russian`) по `title`+`content`; префикс `token:*` (от 3 символов); теги — только через фильтр в сайдбаре.

### Автотесты
- [x] `NoteRepositorySearchTest`, `NoteSearchCaseInsensitiveTest` — PHPUnit green
