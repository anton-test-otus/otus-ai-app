...
### Smoke
- [ ] **Dashboard** — карточки заметок: даты «Сегодня» / «Вчера» / «N дн. назад» / locale; подзаголовок «N заметок» с корректным склонением
- [ ] **`/favorites`** — те же форматы дат на карточках; подзаголовок «N заметок»
- [ ] **Корзина** — «Удалена сегодня» / «вчера» / «N дн. назад» (строчные относительные метки в середине фразы)
- [ ] **Dropdown папок** (NoteView, перемещение заметки) — иерархия и отступы как раньше; parent-only режим не ломается
- [ ] **Фильтр по тегам + папка** — смена фильтров перезагружает списки notes/tags без смешивания (общий `buildFilterCriteriaKey`)
- [ ] **Login / Register** — успешный вход сохраняет token, пользователь авторизован (регрессия `applyAuthResponse`)
