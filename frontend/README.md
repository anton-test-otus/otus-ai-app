...
В Docker переменные задаются в **корневом `.env`** и попадают во `frontend/.env` через `make env` / `scripts/generate-env.sh` (только ключи `VITE_*`).

- `VITE_API_URL` — URL бэкенд API (`http://localhost:8080/api` в dev, `/api` в demo)
- `VITE_AUTH_ENABLED` — если не задан, берётся из `APP_AUTH_ENABLED` при генерации env
...
JWT токены хранятся в localStorage и автоматически добавляются к каждому запросу в `api/client.ts` (заголовок `Authorization: Bearer …`, refresh при 401).
...
См. [PHASES.md](../PHASES.md) — фазы 1–17 (CRUD заметок, папки, теги, поиск, wiki-ссылки, версии, корзина, админка, DnD и др.).
