...
- [ ] `disable` / `delete`: запретить действие над `$user === $this->getUser()` (или требовать другого активного админа)
- [ ] `demote`: не снимать `ROLE_ADMIN`, если это последний администратор в системе
- [ ] Ответ: `400` / `409` с сообщением, не silent success
- [ ] Smoke: единственный админ не может удалить себя
