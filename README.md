...
### Demo / prod деплой (один URL, для проверки)
...
```bash
# 1. Env (единый файл в корне; backend/.env и frontend/.env генерируются при build)
cp .env.example .env
# Отредактируйте .env: APP_SECRET, DB_PASSWORD, JWT_PASSPHRASE, ADMIN_* и т.д.
# prod/demo: VITE_API_URL=/api
./scripts/generate-env.sh

# 2. Frontend-артефакты (frontend/dist) — нужны ДО сборки nginx
# подтянуть готовый dist с ветки dist (CI), без npm
...
- **nginx** — раздаёт `frontend/dist` (шаг 2) и проксирует `/api` в Symfony
- **php (entrypoint)** — миграции; при `APP_AUTH_ENABLED=true` — demo seed; при `false` — `app:ensure-single-user`
- **postgres** — БД в `volumes/postgres`

**Смена `APP_AUTH_ENABLED`** → пересобрать `frontend/dist` (шаг 2) и `docker compose build nginx`.

#### Способ 2 — Make (то же окружение, одной командой)

```bash
cp .env.example .env
# отредактировать .env
make init-prod   # env + frontend/dist + build + up
# опционально: make admin
```

`make init-prod` = `env` + `frontend-dist` + `docker compose build` + `up` + bootstrap (migrate + demo seed).

#### Артефакты frontend (CI / ветка `dist`)

На `main` каталог `frontend/dist` в `.gitignore`. CI ([`.github/workflows/build.yml`](.github/workflows/build.yml)) после успешного CI собирает dist и пушит в ветку **`dist`** (`frontend/dist` + `.dist-source-sha`).
...
cp .env.example .env   # или сохраните свой .env
./scripts/generate-env.sh
...
# + composer install, migrate, admin — см. Makefile init-dev
```

- API: http://localhost:8080/api · UI: http://localhost:5173
- Demo seed в dev **не** в entrypoint — вручную: `docker compose exec php bin/console app:seed-demo-data`

### Настройка для разработчика (dev-режим)

1. Клонируйте репозиторий и перейдите в директорию проекта

2. **Инициализация dev-окружения:**
```bash
make init-dev
```
...
VITE_API_URL=http://localhost:8080/api   # dev; prod/demo: /api
```
...
В корневом `.env`: `APP_AUTH_ENABLED=false`. Compose прокидывает его в php (runtime); для prod-сборки фронта — пересобрать `frontend/dist` с `VITE_AUTH_ENABLED=false` (шаг 2 demo-деплоя или `make frontend-build`). Дополнительно UI определяет режим по `GET /api/auth/me` без токена.
